...
CREATE TABLE `banner_item` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `img_id` INT NOT NULL COMMENT '外键，关联image表',
    `key_word` VARCHAR(100) NOT NULL COMMENT '执行关键字，根据不同的type有不同含义',
    `type` TINYINT NOT NULL DEFAULT '1' COMMENT '跳转类型，可能导向商品，可能导向专题页',
    `banner_id` INT NOT NULL COMMENT '外键，关联banner表',
    `delete_time` INT DEFAULT NULL,
    `update_time` INT DEFAULT NULL,
    PRIMARY KEY (`id`)
...
CREATE TABLE `product` (
  ...
  `img_id` INT DEFAULT NULL COMMENT '图片外键',
  PRIMARY KEY (`id`)
...
CREATE TABLE `category` (
  ...
  `topic_img_id` INT DEFAULT NULL COMMENT '外键，关联image表',
  `delete_time` INT DEFAULT NULL,
...
CREATE TABLE `user` (
  ...
  `delete_time` INT DEFAULT NULL,
  `create_time` INT DEFAULT NULL COMMENT '注册时间',
  `update_time` INT DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `product_image` (
  ...
  `delete_time` INT DEFAULT NULL COMMENT '状态，主要表示是否删除，也可以扩展其他状态',
  `order` INT NOT NULL DEFAULT '0' COMMENT '图片排序序号',
